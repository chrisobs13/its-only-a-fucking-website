...
<h1>liftoff <a href="http://travis-ci.org/tkellen/node-liftoff"><img src="https://secure.travis-ci.org/tkellen/node-liftoff.png" alt="Build Status" /></a> <a href="https://ci.appveyor.com/project/tkellen/node-liftoff/branch/master"><img src="https://ci.appveyor.com/api/projects/status/5a6w8xuq8ed1ilc4/branch/master?svg=true" alt="Build status" /></a></h1>
...
<h2>What is it?</h2>
<p><a href="http://weblog.bocoup.com/building-command-line-tools-in-node-with-liftoff/">See this blog post</a>, <a href="http://github.com/tkellen/node-hacker">check out this proof of concept</a>, or read on.</p>
...
<p>An example utilizing all options:
<code>js
...
<p>In this example Liftoff will look for <code>myappfile{.js,.json,.coffee}</code>.  If a config with the extension <code>.coffee</code> is found, Liftoff will try to require <code>coffee-script/require</code> from the current working directory.
<code>js
...
<p>In this example, Liftoff will look for <code>.myapp{rc}</code>.
<code>js
const MyApp = new Liftoff({
  name: 'myapp',
  configName: '.myapp',
...
<p><strong>Matching CLI Invocation:</strong>
<code>
myapp --cwd ../
</code></p>
...
<p>These are functionally identical:
<code>
myapp --myappfile /var/www/project/Myappfile.js
myapp --cwd /var/www/project
</code></p>

<p>These can run myapp from a shared directory as though it were located in another project:
<code>
